...
pf_glbl.src=[];	
pf_glbl.remove_list_action=[]
	pf_internal_libry={};
	pf_internal_libry.config=[];
	pf_internal_libry.logic={};	
	pf_glbl.report={};
	pf_glbl.report.loadjs=[];
	pf_glbl.report.loadjs_error=[];
	pf_glbl.control={};

	pf_glbl.control.extend={};
	pf_glbl.control.event_act_extend=[];
	pf_glbl.control.extendview={};
	pf_glbl.control.loadlibrary={};
	pf_glbl.control.delegation_record_list=[];
	pf_glbl.control.delegation_list={};
	pf_glbl.control.delegation_list_spli_event=[];
	var main_dom={
...
var creat_elem=document.createAttribute("style");
	...
	this.loopstyle=function(dom,style,intt){
		...
		if(_pf.getJSONtypeof(prop)==="array"){
		var golb_ret={};
			for(var fn in prop){
		...
		else if(_pf.getJSONtypeof(prop)==="string"){
		...
		checkurlvalid_ajax:function(s){
			if(/^(http|https|sftp|ftp|tcp|\/{2}){1,4}/.test(s.toLocaleLowerCase())){
				...
				if(!_pf.has(pf_glbl.config)){
			
			var proto=window.location.protocol;
			var url=window.location.host;
				return main_dom.url_filter_restriction(proto+"//"+url+"/"+s);
				}else{
					var url_m=pf_glbl.config.url;
       ...
       var lcwlk = function( _parent ) {
        if (!!_parent) {
        	gleft += _parent.offsetLeft;
        	gtop += _parent.offsetTop;
        	lcwlk( _parent.offsetParent );
        } else {
        	return rect = {
...
func_type["success"](((jsons==="false")?xhr.responseText:_pf.parseJson(xhr.responseText)),xhr);
}
if(!_pf.has(func_type,"success") && !_pf.has(func_type,"error")){
...
func_type["error"](((jsons==="false")?xhr.responseText:_pf.parseJson(xhr.responseText)),xhr);
}
}							}
						 }
							})("on"+ie[i]);
...
func_type["error"](((jsons==="false")?xhr.responseText:_pf.parseJson(xhr.responseText)),xhr);
		...
		try{
if(_pf.indexOf(["get"],method)>=0){
	xdr.open(method,js_url+"?"+parameter,false);
	xdr.send(dataform); 
	}
else if(_pf.indexOf(["post"],method)>=0){
	...
	if(!_pf.has(dataform)){
		if(_pf.indexOf(arr_ref,"Content-type")==-1){
	...
	xdr.send(dataform);
		...
		is_where_element:function(tar_m,ar){
			var node=[];
			 var node=_pf.clone(ar);
			 var	attr_type=((_pf.getJSONtypeof(tar_m)=="array")?tar_m:[tar_m]);
			 
			 ar.splice(0,ar.length);
			 for (var i in node) {
         
				var ct=0;
				_pf.each(attr_type,function(tk,tv){
						if (_pf.has(node[i],tv)) {
	...
	event_loop:function(elthis,c1,c2,c3,func,act_bool){
		...
		var var_elthis = elthis;
		 elthis.each(function(td,elemm){
		 	var_elthis=elemm[td];
		if(act_bool){
		 main_dom.actionevent(elemm[td],c1,c2,c3,func);

		 return elthis;
		}
		else{

			 pf_glbl.remove_list_action.push(elemm[td])
			 return elthis;
		}
		});
			return var_elthis;
	...
	elem_delegate_event:function(agentype,agentversion,elem,evnt,func,bools){
					...
					if (agentype && parseInt(agentversion)>10){

						var get_del_act=pf$(elem).getattr(['data-counter'])||false;
							if(get_del_act===false){
						pf$(elem).on(evnt,func);
							pf$(elem).setattr({'data-counter':'true'});	
						}
					}else{
						
						var cnt_e=0;
						pf$(elem).on(evnt,function(e){
						...
						//	};
							...
							if (e.target){
								var elem_index = _pf.indexOf(pf_glbl.control.delegation_record_list,e.target);
								
								if(elem_index==-1){
										func.call(this,e);
										pf_glbl.control.delegation_record_list.push(e.target);
								}
	...
	actionevent:function(elems11,ch,ie,mo,func){		
		...
		(function(elems11,ff,mob){elems11.attachEvent(ie,function(se){
			if ( _pf.indexOf(pf_glbl.remove_list_action,elems11)==-1)
		...
		if(typeof(e.targetTouches)!=="object" && _pf.indexOf(pf_glbl.remove_list_action,elems11)==-1){
						   ...
						   var m2_split_count=m2.split(";");
							...
							_pf.each(m2_split_count,function(sck,scv){
							  var m2_split_cnt=scv.split("=");
								...
								if(_pf.count(m2_split_cnt)==1){
									fl_va_attr_all.push(m2_split_cnt[0]);
								}
							if(_pf.count(m2_split_cnt)>=2){
				...
				if(_pf.has(node[ni].getElementsByTagName(fl_m_tot))){
					...
					var node_elem=node[ni].getElementsByTagName(fl_m_tot);
					 for(var i=0,j=node_elem.length; i<j; i++){
							...
							if(fl_type=="where"){
								if(_pf.count(fl_va_cntnt)>0 || _pf.count(fl_va_cntnt_all)>0 || _pf.count(fl_va_attr_all)>0 ){
     ...
     for(var i1=0,j1=els.length; i1<j1; i1++){
 		...
 		_pf.each(tar_m_split,function(cek,cev){
	...
	pf_library.prototype.setMethod=function(value,value2){
			main_dom.setLibrary(value,value2);
		}
	}
	...
	pf_extender.prototype.dom={
		...
		tag_value:function(tar,ar){
			...
			_pf.each(tar_sub,function(eck,ecv){
...
this.request_header={};
	...
	pf_ajax.prototype.abort=function(bools){
		
	//	ajx.xhr.onreadystatechange = null;
	//	ajx.xdr.send();
		
		this.ajx.xdr.abort();
		pf_glbl.is_ajax_abort=true;
		var loc_bools=bools||true;
		if (loc_bools==true)
		this.ajx=main_dom.ajax_controller(this.iecross);
		return this;
	}
	pf_ajax.prototype.ajax=function(config){ 
...
try{
	...
	for(var f1 in ajax_type){
		(function(m,pf_ajax){
			pf_ajax.prototype[m]=function(value,func,bool){
				var jsn_fn={};
				jsn_fn["method"]=m;
				jsn_fn["value"]=value||{};
...
pf_ajax.prototype.returnvalue=function(value_s,func,bool){
	...
	pf_glbl.src.push(main_dom.checkurlvalid(d));
	...
	var pf = document.createElement('script'); 
		...
		pf_glbl.report.loadjs.push(pf.src);
		};
		
		pf.onerror=function(e){
			loadd=true;
		
			pf_glbl.report.loadjs_error.push(pf.src);
			};
...
if(document.readyState==="complete"){
	...
	if(typeof(func)==="function"){func();}
		...
		var setds=setInterval(function(){
			...
			if(_pf.indexOf(pf_glbl.report.loadjs,eroor)>-1 || _pf.indexOf(pf_glbl.report.loadjs_error,eroor)>-1){
					
	var ref_succsee=_pf.indexOf(pf_glbl.report.loadjs,eroor);
	var ref_error=_pf.indexOf(pf_glbl.report.loadjs_error,eroor);
  ...
  for ( i in pairs ) {
    ...
    pair = pairs[i].split("=");    
    if(!_pf.has(obj[decodeURIComponent( pair[0] )])){
         obj[ decodeURIComponent( pair[0] ) ] = decodeURIComponent( pair[1] );
    }else{
        if(_pf.getJSONtypeof(obj[ decodeURIComponent( pair[0] ) ])=="array"){
  ...
  return obj;
	...
	this.checkversion=function(d){
		...
		return _pf.has(d)?(_pf.has(vers,d)?vers[d]:0):vers;
	}
	};
...
this.css=function(d){
	...
	this.inserthtml=function(com,htm){
		...
		this.each(function(td,meth){
                     try{
			if(_pf.has(lst_adj,com) && _pf.has(meth,td))
	...
	this.each(function(td,meth){
	html_form=meth[td];
	 valr_oupt=((_pf.count(form_name)>1)?(valr[td]={}):valr);
	 
	for(var i=0;i<_pf.count(html_form);i++){
		...
		if(_pf.indexOf(type_opt,html_form[i].type)>-1){
			...
			if(_pf.has(html_form[i].name)){
				if(_pf.has(html_form[i].value) ){
				if(html_form[i].value.trim().split("").length>0){
					glb_arry[html_form[i].name]=pf$(html_form[i]).val();		
				}	
				else{
				var alt_val=main_dom.get_attr(html_form[i],"pf_alt_value");
				if(_pf.has(alt_val,"pf_alt_value")){
				glb_arry[html_form[i].name]=alt_val.pf_alt_value;	
...
var evnt_splt=evnt.split(",");
	var main=this;
	var agentype = pf("agenttype").useragent({browser:'msie'});
	var agen_version = pf("agenttype").checkversion('msie');//get_default_browser();
//,mousemove,touchmove,
		...
		pf$(target_element).each(function(ek,ev){
			main_dom.elem_delegate_event(agentype,agen_version,ev[ek],evnt,func,false);
...
var main_helper={
		...
		group:function(type,jsn,val){
			...
			_pf.each(jsn,function(kk,vv){
				...
				if(_pf.getJSONtypeof(vv)==="json"){
				   if(_pf.has(vv[val])){
			
					if(!_pf.has(jsn_val[vv[val]])){
					  jsn_val[vv[val]]=[];
					  jsn_val_count[vv[val]]=[];
                                         
					}
					jsn_val[vv[val]].push(vv);
                                        jsn_val_count[vv[val]]=_pf.count(jsn_val[vv[val]]);
				   } 		
				}
				else{
				  if(!_pf.has(jsn_val[vv])){
				
				     jsn_val[vv]=[];
				      jsn_val_count[vv]=[];
				   }	
				    jsn_val[vv].push(vv);
                                     jsn_val_count[vv]=_pf.count(jsn_val[vv]);		
				}
...
_pf.JSONVariableNotExist=function(jsn,jsn_rpl){
...
else if(_pf.getJSONtypeof(s)==="json"){
...
_pf.template_html=function(domm,reg){
	...
	if(/(\=+\>)/.test(domm)){
			console.log("1");
			var elem_splt=domm.split("=>");
			var t_html=pf$(elem_splt[0].trim()).html();
		
			var ele_attr=elem_splt[1].trim();
			
			var elem_tag=ele_attr==null?elem_splt[1].trim():elem_splt[1].split("[")[0].trim();
			var el   = document.createElement("div");
			el.innerHTML = t_html;

				pf$(el).findElem(ele_attr).each(function(ek,ev){
				dom_main+=pf$(ev[ek]).html().replace(/(--){0,}/gi,"").replace(/\=('|")(<!.*?!>)('|")/g,"=$2");
				});
			
		
	}else{

		 dom_main=pf$(domm).html();
...
if(get_json=="object" && _pf.has(jsn,"style") && _pf.has(jsn,"nodeType")  && _pf.has(jsn,"ownerDocument")){
for(var i in jsn){
...
window._pf_t=function(){
						console.log("PF templates");
			};
	
	_pf_t.input_value=function(jsn,type,elem){
			var types=_pf.has(type)?type:"name";
			var elems=_pf.has(elem)?elem:"all:";
			var jsns=jsn||{};
			_pf.each(jsns,function(jk,jv){
						
				var eleme_te=pf$(elems+"["+types+"="+jk+"]");
	          
	                       if(_pf.getJSONtypeof(eleme_te.getTagName())=="string"){
				if(eleme_te.getTagName()=="SELECT"){
	...
	_pf_t.isValueEmpty=function(val){
		var elem_len=pf$(val).getlength();
		var cnt_len=0;		
		pf$(val).each(function(ek,ev){
				var val_aut=pf$(ev[ek]).val();
				if(val_aut.split("").length>0){
					cnt_len++;
				}
			});
		return cnt_len==elem_len;
	}
...
var pf_mvc_support={
		template_html:function(dom,val,rsv_jsn){
			alert("SAd");
		}
		};
var pf_core_library={
	        init_mvc_support:function(dom,return_val,rsrv_json){
			if(_pf.has(return_val) && _pf.getJSONtypeof(return_val)==="json"){
				
				for(var ti in return_val){
					if(_pf.has(pf_mvc_support,ti)){
						pf_mvc_support[ti](dom,return_val[ti],rsrv_json);
					}
				}
			}
		},     
		...
		var globlas={
		set:function(ky,val){
		if(!_pf.has(pf_glbl.global_setting))
	pf_glbl.global_setting={};
		try{	
			
			if(typeof(ky)!="object"){
			
		pf_glbl.global_setting[ky]=val;
		}
	else	
		pf_glbl.global_setting=ky;
	...
	console.log("date can`t log");
		...
		insert:function(where,val){
			...
			var ech=pf_glbl.global_setting;
		...
		get_list:function(){
			var cntn=[];
			_pf.each(pf_glbl.global_setting,function(k,v){
				cntn.push(k);
			});
			return cntn;
		...
		dom_event_trigger:function(){
			var loop_act = pf_glbl.control.event_act_extend;
			

		var agentype = pf("agenttype").useragent({browser:'msie'});
	var agen_version = pf("agenttype").checkversion('msie');
			_pf.each(loop_act,function(jk,jv){
				var jsn_val = jv.jsn_val;
				var arr_action = jv.arr_action;
				
				var controller_name = jv.controller_name;
				var pf_view = jv.pf_view;
				var type = jv.type;
					...
					if(k_s.length==2){
						if(_pf.getJSONtypeof(v)==="string"){
							...
							jsn_bol=true;
								...
								main_dom.elem_delegate_event(agentype,agen_version,k_s[0],k_s[1],function(e){
									...
									if(_pf.has(pf_view.prototype.view[spl_v])){
										...
										if(_pf.has(arr_action[spl_v])){
											...
											pf_view.prototype.view.__renderID__=arr_action[spl_v];
			                                         ...
			                                         var rsv_jsn={};
								var ret=pf_view.prototype.view[spl_v].call(pf_view.prototype.view,e);
								pf_core_library.init_mvc_support(pf_view.prototype.view,ret,rsv_jsn);	
	...
	var dom_main=document.body.getElementsByTagName("*");
		...
		load_cls_renderView:function(domss,bool){
			...
			var libb=pf_glbl.control.loadlibrary;
		...
		load_delegation_eventtype:function(cntt){
			...
			var libb=pf_glbl.control.delegation_list;
			var outpt=[];
...
function pf_external(){
		...
		this.cls_event=function(cls,mthd,para){
			...
			var cls_var=para||{};
			if(_pf.has(pf_glbl.control.extendview[cls])){
				if(_pf.has(pf_glbl.control.extendview[cls][mthd])){
					pf_glbl.control.extendview[cls][mthd].call(cls_var);
...
pf_init.prototype.Collection.addModel=function(key,val){
	pf_glbl.control.extend[key]=val;
}
pf_init.prototype.Collection.getModel=function(key,val){
	if(_pf.has(pf_glbl.control.extend[key])){
	return pf_glbl.control.extend[key];
	}else{
		return null;
	}
}
...
pf_init.prototype.Collection.getView=function(key){
	if(_pf.has(pf_glbl.control.extendview[key])){
	
	
	return pf_glbl.control.extendview[key];
	}else{
		return null;
	}
}
window.pf$view=pf_init.prototype.Collection.getView;
...
pf_glbl.control.event_act_extend.push({"arr_action":arr_action,"jsn_val":jsn_g,"controller_name":cntlr,"type":"controller","pf_view":pf_view});
pf_core_library.dom_event_trigger();
}
...
try{
		...
		var cls=_pf.JSONVariableNotExist({},classes);
		pf_glbl.control[ldb][cntlr_m]=cls;
...
pf_view.prototype.allowView=function(cls_glb,classes){
	var bool_glb=cls_glb['allow']||true;
...
try{
	if(bool_glb==true && _pf.has(cls_glb['method']) ){
		...
		pf_glbl.control.extendview[cls_name]=cls;
	}
}catch(e){console.log(e);}
}
		...
		_pf.each(jsn_g,function(k,v){
			...
			var s_v=v.split("|");
				...
				if(_pf.has(pf_view.prototype.view[ss_v])){
					...
					var rsv_jsn={};
					var ret=pf_view.prototype.view[ss_v].call(pf_view.prototype.view,rsv_jsn);
					pf_core_library.init_mvc_support(pf_view.prototype.view,ret,rsv_jsn);
...
var event_splt=pf_glbl.control.delegation_list_spli_event;
	...
	pf$(document.body).on(event_splt.join(","),function(e){
		...
		pf_glbl.control.delegation_list_spli_event=_pf.append_isArrayExist(event_splt,new_event_splt);
	...
	if (e.target){
		var elem_indexof = _pf.indexOf(pf_glbl.control.delegation_record_list,e.target);
		if(elem_indexof>-1){
			pf_glbl.control.delegation_record_list.splice(elem_indexof,1);
		}
	}	
